...
class WaveletModel(ClassificationModel):
    def __init__(self, name, n_classes,  freq, outputfolder, input_shape, regularizer_C=.001, classifier='NN', augment=False, class_balancing=False, pos_weight_cap=10.0):
    ...
    def fit(self, X_train, y_train, X_val, y_val):
        ...
        if self.classifier == 'NN':
            ...
            self.model.fit(XFT_train, y_train, validation_data=(XFT_val, y_val), epochs=self.epochs, batch_size=128, verbose=0, callbacks=[mc_loss, FastaiStyleCallback(self.name)])#, mc_score])
    ...
    def predict(self, X):
        ...
        if self.classifier == 'NN':
            ...
            XFT = ss.transform(XF)
            model = load_model(self.outputfolder+'best_loss_model.h5', compile=False)#'best_score_model.h5', custom_objects={'keras_macro_auroc': keras_macro_auroc})
