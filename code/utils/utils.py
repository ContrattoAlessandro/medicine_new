...
def compute_fmax(y_true, y_pred):
    ...
    best_f1 = 0.0
    t_sums = y_true.sum(axis=1)
    
    for tau in thresholds:
        preds = (y_pred >= tau)
        p_sums = preds.sum(axis=1)
        
        intersection = (preds & y_true.astype(bool)).sum(axis=1)
        
        # Precision sample-centric
        has_pred = p_sums > 0
        if has_pred.any():
            prec = (intersection[has_pred] / p_sums[has_pred]).mean()
        else:
            prec = 0.0
            
        # Recall sample-centric
        rec = (intersection / (t_sums + 1e-8)).mean()
...
def load_dataset(path, sampling_rate, release=False):
    if os.path.basename(os.path.normpath(path)) == 'ptbxl':
    ...
    elif os.path.basename(os.path.normpath(path)) == 'ICBEB':
        # load and convert annotation data
        ...
        for e in exps:
            ...
            try:
                ...
                if 'Fmax' in me_res.columns:
                    ...
                    results_dic[e+'_F1'].append("%.3f(%.2d)" %(np.round(mean2,3), int(unc2*1000)))
                else:
